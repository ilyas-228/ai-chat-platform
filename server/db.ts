...
export async function insertChatMessage(
  ...
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot insert chat message: database not available");
    return null;
  }
...
export async function clearChatMessages(userId: number) {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot clear chat messages: database not available");
    return null;
  }
  ...
  try {
    ...
    console.error("[Database] Failed to clear chat messages:", error);
    throw error;
  }
}
